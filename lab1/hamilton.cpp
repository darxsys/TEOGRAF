...
/**
    Funkcija vrsi konacnu provjeru je li graf iskljucivo skoro hamiltonovski,
    a ne ujedno i hamiltonovski.

    @param 
    graph - matrica susjedstva
    mask - najveca moguca maska za trenutni graf. Uloga
    maske objasnjenja u sljedecoj funkciji.
    N - broj vrhova trenutnog grafa.

    @return Vraca je li graf iskljucivo skoro hamiltonovski ili ne.
*/
bool check_if_not_hamiltonian(int *graph, lld mask, int N) {
    int current = -1;
    mask--;
    for (int i = 0; i < N; i++) {
        if (dp[mask * N + i] == 1) {
...
int check_hamiltonian(int *graph, int N) {
    ...
    // konacna provjera
    bool status = check_if_not_hamiltonian(graph, mask, N);
    if (status == false) {
        result = 0;
    }
    return result;
